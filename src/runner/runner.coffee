...
module.exports =
  ...
  processFiles: (options, cb) ->
    ...
    wrench.readdirRecursive options.src, (error, files) =>
      ...
      return merge(options, processingResult, cb) if error == files == null
...
merge = (options, {errors, warnings, lang}, cb) ->
  if errors.length > 0
...
appliableOptions = (options) ->
  _.pick(options, "textAsKey", "wordsLimitForKey", "replaceSpaces", "translateMe")
